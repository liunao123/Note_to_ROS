...
class PoseSearcher:
    ...
    def search_poses_by_distance(self, 
        ...
        query = """
            ...
            LEFT JOIN data_sessions ds ON op.session_id = ds.session_id
                WHERE 
                    ...
                    -- 步骤3: 精确的圆形范围过滤
                    AND SQRT(
                        POWER(op.position_utm_x - (SELECT utm_x FROM target_utm), 2) + 
                        POWER(op.position_utm_y - (SELECT utm_y FROM target_utm), 2)
                    ) <= %s
            )
            SELECT 
                pd.*,
        ...
        # 将session过滤添加到pose_data的WHERE子句中
        query = query.replace(
            ") <= %s",
            f") <= %s{session_filter}"
        )
