...
std::string constructUTMProjectionWKT(double longitude, double latitude)
{
    ...
    std::string wkt = "PROJCS[\"WGS 84 / UTM zone " + std::to_string(zone) + hemisphere + "\","
                      ...
                      std::to_string(central_meridian) + "],"
...
// 遍历指定的子文件夹，返回指定后缀的文件路径
// extension: 文件后缀，如".pcd", ".txt", ".yaml"等。如果为空字符串则返回所有文件
// 支持三层目录结构: root_dir/middle_dir/target_subdir/files
std::vector<std::string> getAllFilePaths(const std::string &root_dir, const std::string &extension = "")
{
    ...
    try
    {
        ...
        target_subdirs.push_back("sparse/vehicle_geo_pose");

        // 遍历第一层目录（中间层目录，如cam_all_2025-11-21-14-24-53_0）
        for (const auto &middle_entry : fs::directory_iterator(root_dir))
        {
            if (middle_entry.is_directory())
            {
                ...
                // std::cout << "Found middle directory: " << middle_dir_path << std::endl;
                ...
                for (const std::string &subdir : target_subdirs)
                {
                    ...
                    // std::cout << "Scanning directory: " << full_subdir_path << std::endl;
...
int main(int argc, char **argv)
{
    ...
    std::cout << "all_pose_files.size(): " << all_pose_files.size() << std::endl;
    std::cout << "all_pcd_files.size(): " << all_pcd_files.size() << std::endl;
    ...
    if (!pose_cloud->points.empty())
    {
        ...
        for (int idx : found_indices)
        {
            ...
            if (idx < all_pose_files.size())
            {
                ...
                std::cout << "  read file: " << 100*count++ / found_indices.size() << "%" << std::endl;
        ...
        std::cout << "Filtered out " << (original_count - filtered_count) << " points beyond range" << std::endl;
        ...
        // 将经纬度转换为字符串并构造新的输出文件名
        std::ostringstream coord_stream;
        coord_stream << std::fixed << std::setprecision(6) << select_point_lon << "_" << select_point_lat;
        std::string coord_string = coord_stream.str();
        
        // 在原文件名中插入坐标信息
        std::string modified_output_las = output_las;
        size_t dot_pos = modified_output_las.find_last_of('.');
        if (dot_pos != std::string::npos) {
            modified_output_las.insert(dot_pos, "_" + coord_string);
        } else {
            modified_output_las += "_" + coord_string + ".las";
        }
        
        std::cout << "Output file with coordinates: " << modified_output_las << std::endl;

        writeLas(*filtered_map_cloud, modified_output_las, OFFSET_X, OFFSET_Y, OFFSET_Z, false, select_point_lon, select_point_lat);
    }

    std::cout << "----------------------------------------" << std::endl;
    // std::cout << "Total files found: " << all_pose_files.size() << std::endl;
    // std::cout << "Total pose_data stored: " << all_pose_data.size() << std::endl;
    std::cout << "Final Point Cloud Size : " << filtered_map_cloud->size() << " points" << std::endl;
    
    // 构造带坐标的文件名用于显示
    std::ostringstream coord_stream_final;
    coord_stream_final << std::fixed << std::setprecision(6) << select_point_lon << "_" << select_point_lat;
    std::string coord_string_final = coord_stream_final.str();
    std::string final_output_name = output_las;
    size_t dot_pos_final = final_output_name.find_last_of('.');
    if (dot_pos_final != std::string::npos) {
        final_output_name.insert(dot_pos_final, "_" + coord_string_final);
    } else {
        final_output_name += "_" + coord_string_final + ".las";
    }
    
    std::cout << "Output_las: " << final_output_name << " saved successfully." << std::endl;
    std::cout << "----------------------------------------" << std::endl;
